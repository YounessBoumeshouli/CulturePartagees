...
<article class="w-64 transition-all m-6 ease-in-out duration-150 border-4 border-transparent rounded-lg p-2 cursor-pointer bg-white hover:shadow-[10px_10px_0_#4e84ff,20px_20px_0_#4444bd] hover:-translate-x-5 hover:-translate-y-5 hover:border-[#0578c5] active:shadow-none active:translate-x-0 active:translate-y-0">
  ...
  <div class="flex flex-col gap-5 p-2 pt-5">
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-semibold text-black truncate m-0"><?php 
      echo $Author["nom"]." ".$Author["prénom"];
      ?></h2>
      <a href="index.php?action=changestatus&id_membre=<?=$Author["id_user"]?>" class="w-12 h-12 rounded-full p-2 transition-transform ease-in-out duration-300 bg-white hover:bg-[#a6c2f0] hover:-rotate-45">
    ...
    <div class="flex gap-2">
      ...
      <span class="px-3 py-1 text-xs font-bold rounded-full bg-[#b2b2fd] text-[#1a41cd]"><?=$Author["status"]?></span>
    </div>
  </div>
</article>
