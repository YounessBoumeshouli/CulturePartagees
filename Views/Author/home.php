    ...
    <div class="container mx-auto px-6 py-10">
      ...
      <div class="mt-8 grid grid-cols-1 gap-8 md:mt-16 md:grid-cols-2 xl:grid-cols-3">
        ...
        foreach ($result as $row) {
            ...
            $imageData = $row["article_image"] ?? null;
            $mimeType = $row["mime_type"] ?? 'image/jpeg'; // Default MIME type if not provided

            // Prepare the image source
            $imageSrc = 'path/to/default/image.jpg'; // Default image if no image data is found
            if ($imageData) {
                if (is_resource($imageData)) {
                    $imageData = stream_get_contents($imageData); // Convert resource to string
                }
                $imageDataEncoded = base64_encode($imageData);
                $imageSrc = 'data:' . $mimeType . ';base64,' . $imageDataEncoded;
            }
            $imageDataAuthor = $row["profile_image"] ?? null;
            $mimeTypeAuthor = $row["mime_type"] ?? 'image/jpeg'; // Default MIME type if not provided

            // Prepare the image source
            $imageSrcAuthor = 'path/to/default/image.jpg'; // Default image if no image data is found
            if ($imageDataAuthor) {
                if (is_resource($imageData)) {
                    $imageDataAuthor = stream_get_contents($imageData); // Convert resource to string
                }
                $imageDataEncoded = base64_encode($imageData);
                $imageSrcAuthor = 'data:' . $mimeType . ';base64,' . $imageDataEncoded;
            }
        ?>
        <div>
          <div class="relative">
            ...
            <div class="absolute bottom-0 flex bg-white p-3 dark:bg-gray-900">
              <img
                ...
                width="" />
              <div class="mx-4">
                <h2 class="text-sm text-gray-700 dark:text-gray-200"><?=$row["nome"]?></h2>
                <p class="text-sm text-gray-500 dark:text-gray-400"><?=$row["prénom"]?></p>
              </div>
            </div>
          </div>
