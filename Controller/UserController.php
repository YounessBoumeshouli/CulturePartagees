...
function ajouterArticleAction() {
    if (isset($_POST["content"], $_POST["title"], $_POST["categorie"])) {
        ...
        $row = $Author->addArticle($categorie, $title, $content, $_SESSION["user"]["id_user"],$imageData);
...
function memebersList(){
    ...
    function Login(){
        if(isset($_POST["email"], $_POST["password"])){
            $email = $_POST["email"];
            $password = $_POST["password"];    
            $user = new User(null, null, $email, null, null, null, null, Database::getConnection());
...
function SignUp(){
    if(isset($_POST["name"])){
        ...
        $user = new $role($nom, $prenom, $email, $hashedPassword, $phone, $role, $registrationDate, Database::getConnection());
        $user->Inscription();
    }
}
